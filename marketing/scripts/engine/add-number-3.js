#!/usr/bin/env node
/**
 * Links WhatsApp number 3.
 * Run in a REAL terminal (not Claude Code):
 *   node marketing/scripts/engine/add-number-3.js
 */
...
const SESSION = path.join(__dirname, '../../leads/sessions/wa-number-3');
...
console.log('  LINKING WHATSAPP NUMBER 3');
console.log('━'.repeat(50));
console.log('1. Open WhatsApp on your THIRD phone');
...
client.on('qr', (qr) => {
  console.clear();
  console.log('━'.repeat(50));
  console.log('  SCAN THIS QR CODE WITH YOUR THIRD PHONE');
  console.log('━'.repeat(50) + '\n');
...
client.on('ready', async () => {
  ...
  console.log('\n🎉 SUCCESS! WhatsApp number 3 linked!');
  console.log(`   Phone: ${info?.wid?.user || 'connected'}`);
  console.log('   Engine now sends via all 3 numbers (150 messages/day).');
...
setTimeout(() => {
  console.log('\n⏰ Timeout — QR not scanned within 5 minutes. Run again to retry.\n');
  process.exit(1);
}, 5 * 60 * 1000);
